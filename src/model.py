...
from base_model import TSN_BASE
import numpy as np
import torch
from torchvision import transforms
from image_util import GroupMultiScaleCrop, GroupRandomHorizontalFlip
...
TSN Configurations:
    ...
    def _prepare_base_model(self):
        self.base_model = TSN_BASE()
        self.base_model.last_layer_name = 'Global_pool'
        self.input_size = 224
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d) or isinstance(
                ...
                if len(list(m.parameters())) > 0:
                    raise ValueError(
                        "New atomic module type: {}. Need to give it a \
                            learning policy".format(type(m)))
